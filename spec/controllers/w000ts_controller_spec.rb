...
JSON
  ...
  it 'should create a w000t with a status as json' do
    ...
    expect do
      post :create, params: {
                      w000t: { long_url: url, status: 'private' },
                      user_id: @user.id
                    },
                    format: :json
    end.to change { W000t.count }.by(1)
  ...
  it 'should not create a w000t with a wrong status as json' do
    expect do
      post :create, params: {
                      w000t: { long_url: 'http://google.fr', status: 'yo' },
                      user_id: @user.id
                    },
                    format: :json
  ...
  it 'should destroy as a logged in user' do
    ...
    expect do
      post :destroy, params: { short_url: @user_w000t.short_url }
    end.to change { W000t.count }.by(-1)
  ...
  it 'should destroy with a user token as json' do
    ...
    expect do
      post :destroy, params: { short_url: @w000t.short_url }, format: :json
    end.to change { W000t.count }.by(-1)
    assert_response :success
  end
  ...
  it 'should not destroy as anonymous user' do
    expect do
      post :destroy, params: { short_url: @w000t.short_url }
    end.to change { W000t.count }.by(0)
  ...
  it 'should not destroy as the wrong user' do
    ...
    sign_in @user
    expect do
      post :destroy, params: { short_url: w000t_by_joe.short_url }, format: :json
    end.to change { W000t.count }.by(0)
  ...
  it 'should update as a logged in user' do
    ...
    patch :update, params: {
                    short_url: @user_w000t.short_url,
                    w000t: { tags: 'test, yo' }
                   },
                   format: :js
    assert_response :success
  ...
  it 'should not update as the wrong user' do
    ...
    sign_in @user
    patch :update, params: {
                     short_url: w000t_by_joe.short_url,
                     w000t: { tags: 'test, yo' }
                   },
                   format: :js
  ...
  it 'should create a w000t with a token as header as json' do
    ...
    expect do
      post :create, params: {
                      w000t: { long_url: 'google.fr', status: 'private' }
                    },
                    format: :json
    end.to change { W000t.count }.by(1)
  ...
  it 'should get user w000t list filtered by type' do
    ...
    TypableUrl::TYPES.each do |type|
      get :owner_list, params: { type: type.to_s }
      assert_response :success
    end
  end
  ...
  it 'should get user w000t list with a filter error' do
    sign_in @user
    get :owner_list, params: { type: 'some shit' }
  ...
  it 'should get 404 on public wall of fake user' do
    get :user_wall, params: { user_pseudo: 'bazooka' }
    assert_response 404
  end
  ...
  it 'should be redirected' do
    ...
    get :redirect, params: { short_url: @w000t.short_url }
    sleep 0.5
  ...
  it 'should get a 404 http code for a fake link' do
    get :redirect, params: { short_url: 'null' }
    assert_response 404
  end
  ...
  it 'should be have one more click' do
    request.headers['REMOTE_ADDR'] = '8.8.8.8'
    before_redirect = @w000t.number_of_click
    get :click, params: { short_url: @w000t.short_url }, format: :js
    after_redirect = W000t.find(@w000t.id).number_of_click
    assert_equal before_redirect + 1, after_redirect, 'Wrong number of click'
    assert_response :success
  end
